...
word=['A','B','C','D']
_word = ['1','2','3','4','5','6','7']
#依据文件命名区分设定的方案，如A1对应MA&UO2厚度d=0.1cm,比例Q=0.1
file = '0501'
#指定文件夹下的所有文件，可依据具体情况修改删除。
filename ="{}数据处理.xlsx".format(file)
name = 'den_tot'
openfile = []
n,b=0,[]
#自定义的寻找file文件下的所有含name的文件
def searchfile(path):
...
print("处理的文件有：")
searchfile(os.path.abspath('.')+'\\'+file)
#调用寻找文件路径函数，此处可更改默认路径
input("请按回车确认执行")
lsin=os.listdir()
# 寻找同目录下的'den_tot'文件
wb=Workbook()
...
#标记在哪一个方案中，A1对应体积V=29.82894964cm3,MA:UO2比值Q=0.1依次类推。
    ...
    for i in range(n):
        sheet.cell(row=m * (x + 2) + i + 3, column=6).value = "=A{}"\
            .format(m*(x+2)+i+3)#day数据填充
        for j in range(len(a)-1):
            sheet.cell(row=m * (x + 2)+i+3, column=j+12).value="={1}{2}*{3}-{1}{0}*{3}"\
                .format(m*(x+2)+i+3,chr(j+66),m*(x+2)+3,V[w]*nrou*M[j]*N/Na)
            sheet.cell(row=m*(x+2)+i+3,column=j+7).value = "=({1}{2}-{1}{0})/{1}{2}"\
                .format(m*(x+2)+i+3,chr(j+66),m*(x+2)+3)
    for j in range(len(a)-1):
        sheet.cell(row=1, column=j+17).value = a[j+1]
        sheet.cell(row=m+2, column=j+17).value = "={1}{0}*{2}"\
            .format(m*(x+2)+n+2,chr(j+71),V[w]*rou*Q[z]*N*F[j]/(Q[z]+1))
# sheet.cell(row=m + 2, column=j + 17).value = "={1}{0}*{2}"
        # .format(m * (x + 2) + n + 2, chr(j + 71),V[w] * rou *N*F[j])
        sheet.cell(row=m+2, column=j+22).value = "={1}{0}*{2}"\
            .format(m*(x+2)+n+2, chr(j+71),100)
